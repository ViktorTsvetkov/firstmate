...
# On native Windows, the same proof is checked with Restart Manager instead of
# lsof: an old lock is removable only when Restart Manager reports no holder for
# the lock file or the worktree directory, and every unavailable/error path
# fails closed.
set -eu
...
restart_manager_paths_have_holder() {
  local ps output status path windows_path windows_paths=()
  if command -v powershell.exe >/dev/null 2>&1; then
    ps=powershell.exe
  elif command -v powershell >/dev/null 2>&1; then
    ps=powershell
  else
    echo "teardown: Restart Manager check failed: powershell not found" >&2
    return 2
  fi
  ...
  for path in "$@"; do
    ...
    if command -v cygpath >/dev/null 2>&1; then
      windows_path=$(cygpath -aw "$path") || {
        echo "teardown: Restart Manager check failed: cannot convert path $path" >&2
        return 2
      }
    fi
    windows_paths+=("$windows_path")
  done

  [ "${#windows_paths[@]}" -gt 0 ] || return 1

  # shellcheck disable=SC2016 # PowerShell script block; Bash must not expand it.
  if output=$("$ps" -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command '
$paths = @($args | Where-Object { $_ })
if ($paths.Count -eq 0) { [Environment]::Exit(1) }
$source = @"
using System;
...
try {
  ...
  $result = [RestartManagerProbe]::RmStartSession([ref]$session, 0, $key)
  if ($result -ne 0) { Write-Error "RmStartSession failed: $result"; [Environment]::Exit(2) }
  try {
    $result = [RestartManagerProbe]::RmRegisterResources($session, [uint32]$paths.Count, [string[]]$paths, 0, [IntPtr]::Zero, 0, $null)
    if ($result -ne 0) { Write-Error "RmRegisterResources failed: $result"; [Environment]::Exit(2) }
    $needed = 0
    $count = 0
    $reboot = 0
    $result = [RestartManagerProbe]::RmGetList($session, [ref]$needed, [ref]$count, $null, [ref]$reboot)
    if ($result -eq 234) {
      $count = $needed
      $apps = New-Object RestartManagerProbe+RM_PROCESS_INFO[] $count
      $result = [RestartManagerProbe]::RmGetList($session, [ref]$needed, [ref]$count, $apps, [ref]$reboot)
    }
    if ($result -ne 0) { Write-Error "RmGetList failed: $result"; [Environment]::Exit(2) }
    if ($count -gt 0) { [Environment]::Exit(0) }
    [Environment]::Exit(1)
  } finally {
    if ($session -ne 0) { [void][RestartManagerProbe]::RmEndSession($session) }
  }
} catch {
  Write-Error $_
  [Environment]::Exit(2)
}
' "${windows_paths[@]}" 2>&1); then
    return 0
  else
    status=$?
  fi
  ...
  if [ -n "$output" ]; then
    printf '%s\n' "$output" | sed 's/^/teardown: Restart Manager check failed: /' >&2
  else
    echo "teardown: Restart Manager check failed with exit $status" >&2
  fi
  return 2
}
...
worktree_lock_has_live_holder() {
  ...
  if fm_platform_is_windows; then
    if restart_manager_paths_have_holder "$lock" "$dir"; then
      return 0
    else
      status=$?
