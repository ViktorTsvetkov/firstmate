...
- Helper scripts in `bin/` are plain bash.
  ...
  The root `.gitattributes` pins `*.sh` files to LF so Git Bash can run the suite on Windows checkouts even when global Git config would otherwise use CRLF.
...
tests/fm-daemon.test.sh                   # sub-supervisor classifier, /afk presence-gating, max-defer, composer, stale dedupe, scan flush, and fm-send submit tests
tests/fm-classify-lib.test.sh             # shared status classifier: leading UTF-8 BOM normalization, proven native-Windows UTF-16 status decoding, captain-relevant matching, and POSIX byte identity
...
tests/fm-send-secondmate-marker.test.sh   # fm-send from-firstmate marker for kind=secondmate task selectors: marked vs crewmate/explicit/--key, default-home selector resolution, explicit-target no-state behavior, and the exact marker byte sequence
...
tests/fm-afk-inject-herdr-e2e.test.sh     # real-herdr end-to-end test of the afk daemon's herdr transport, on an isolated throwaway HERDR_SESSION with agent-state reporting: partial-input deferral, swallowed-Enter retry, a normal digest, and the max-defer wedge alarm on a persistently pending composer
...
tests/fm-crew-state.test.sh               # fm-crew-state.sh current-state reconciliation: run-step authority including closed panes, stale needs-decision/blocked superseded by a resumed run, genuine-parked, cross-branch runs-list attribution, pane fallback, BOM-normalized and native-Windows UTF-16-decoded status-log fallback, scout skip, torn-down/missing-meta graceful
tests/fm-backend.test.sh                  # runtime-backend abstraction: fm-backend.sh selection/meta/dispatch helpers, active-herdr gating for unrecorded three-field explicit targets, shell-portable sourced backend matching, and old-vs-new fake-tool command-log conformance for fm-send/fm-peek/fm-spawn/fm-teardown
...
tests/fm-backend-herdr.test.sh            # fake herdr CLI unit tests for the experimental herdr adapter, including version/tool gates, running-server compatibility refusal after herdr updates, target parsing, explicit peek without raw-selector metadata, send/capture, ANSI-aware composer guards, native agent-state submit confirmation, Windows Codex idle suggestions, native busy state, per-home workspace-label resolution, default-tab prune safety, spawn-abort and post-create partial-pane cleanup, restored-layout husk replacement, and verified CLI bug workarounds
tests/fm-backend-herdr-msys-pathconv.test.sh # focused fake-herdr regression for native-Windows Git Bash MSYS path-conversion guards around leading-slash payload sends and key sends
tests/fm-backend-herdr-handoff.test.sh    # focused fake-herdr regression for the native-Windows spawn handoff: short Git Bash shell path, explicit pane shell start, treehouse get, and active cwd probe
tests/fm-backend-herdr-release.test.sh    # focused fake-herdr regression for native-Windows empty-session release after the last pane closes, including self-session, non-empty-session, and POSIX no-op guards
