...
Four narrow, additive conversions bridge that gap, each gated on `fm_platform_is_windows` (`bin/fm-platform-lib.sh`) so the POSIX path is byte-for-byte unchanged:
...
On POSIX every branch is a no-op: no `cygpath` is invoked, the CR strip is an identity passthrough, the cwd is passed through untouched, and the server launch is the original backgrounded subshell.
