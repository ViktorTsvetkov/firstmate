...
### Partial-task cleanup during spawn aborts

`fm_backend_herdr_create_task` treats a tab or pane created before a later adapter-level failure as partial state, not a live task.
If id parsing, the native-Windows login-shell handoff, or post-husk-replacement verification fails after `tab create`, the adapter closes the just-created pane when it has a pane id, or the just-created tab when only a tab id is known.
At the `fm-spawn.sh` level, herdr also mirrors Orca's abort-cleanup guard: once the task pane target is known, a non-zero exit before `state/<id>.meta` is committed closes that pane through `fm_backend_kill herdr`.
That covers later spawn assertions such as state-directory creation, worktree discovery, and other post-endpoint checks for both ordinary tasks and `--secondmate` launches.
The cleanup is disarmed immediately after metadata is written, so a successful spawn keeps its pane open and normal teardown remains the owner of endpoint cleanup.
This guard is herdr-only and is inert until a herdr endpoint exists, so the tmux path and spawn refusals that happen before endpoint creation are unchanged.
Covered by `tests/fm-backend-herdr.test.sh`'s `test_windows_create_task_closes_partial_pane_when_shell_handoff_fails`, `test_spawn_closes_herdr_pane_when_later_abort_occurs`, `test_spawn_success_keeps_herdr_pane_open`, and `test_secondmate_spawn_closes_herdr_pane_when_later_abort_occurs`.
...
| Send key | `herdr --session <session> pane send-keys <pane> <key>` | Verified names: `enter`, `escape` (alias `esc`), `ctrl+c` (aliases `C-c`, `c-c`). `ctrl+c` verified to interrupt a running foreground process immediately. The `--session` flag must be before `pane send-keys` because `send-keys` has a variadic key tail; a trailing `--session` is consumed as another key token instead of a global selector. On native Windows Git Bash, the adapter disables MSYS path conversion around this call too, keeping the Enter that submits a leading-slash payload on the same literal path. |
...
| Partial-task cleanup | `herdr pane close <pane>` or `herdr tab close <tab>` | Used only after this spawn created a task tab/pane and then aborts before metadata commit, or when `fm_backend_herdr_create_task` itself fails after `tab create`. Closing the root pane closes its tab; `tab close` is only the parse-failure fallback when the pane id is unavailable. |
...
A new `FM_SUPERVISOR_BACKEND` override (`tmux`|`herdr`) resolves independently, mirroring `bin/fm-backend.sh`'s own `fm_backend_detect`: `$TMUX_PANE` set selects tmux (even nested inside herdr, matching the innermost-first rule); `$HERDR_ENV=1` with `$HERDR_PANE_ID` present selects herdr, composing the target as `"${HERDR_SESSION:-default}:${HERDR_PANE_ID}"`; absent those env markers on native Windows, a live Herdr owner in the per-home session lock selects the same herdr backend and target; absent all of those, the daemon falls back to tmux/`firstmate:0`, byte-identical to its pre-herdr-support behavior.
On native Windows with herdr, startup validates an explicit `FM_SUPERVISOR_TARGET` with `fm_backend_target_exists`; if the override is stale but the live `$HERDR_PANE_ID` or session-lock fallback resolves, the daemon logs that it ignored the stale override and starts on the live target.
POSIX and tmux keep the historical explicit-target refusal path.
...
**Afk-clear shutdown.** On native Windows with herdr, the daemon evaluates the `state/.afk` inactive gate on every housekeeping loop tick, not only inside injection.
That lets a cleared away-mode flag stop the daemon promptly even under wake starvation.
POSIX and tmux shutdown timing remains unchanged.
...
Seven narrow, additive conversions bridge that gap, each gated on `fm_platform_is_windows` (`bin/fm-platform-lib.sh`) so the POSIX path is byte-for-byte unchanged:
...
- **MSYS argument path conversion is disabled for literal sends and key sends.** `fm_backend_herdr_send_literal` and `fm_backend_herdr_send_key` set `MSYS_NO_PATHCONV=1` and `MSYS2_ARG_CONV_EXCL='*'` only around the native herdr `pane send-text` and `pane send-keys` calls.
Without that guard, Git Bash/MSYS can rewrite a leading-slash skill payload such as `/no-mistakes` into a Git-install path before the Windows-native `herdr.exe` receives it.
POSIX leaves those environment variables unset.
