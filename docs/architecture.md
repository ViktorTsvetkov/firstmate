...
The shared status-log reader strips only a genuine leading UTF-8 BOM, which native Windows PowerShell 5.1 can write when first creating a UTF-8 status file, before status verbs are classified.
It also decodes UTF-16LE or UTF-16BE on native Windows only when the file bytes prove that encoding by BOM or a consistent null-byte code-unit pattern, leaving ordinary BOM-less UTF-8 and POSIX status lines byte-identical.
...
Routine re-arms go through `bin/fm-watch-arm.sh`, which forks the watcher as a tracked child, verifies it is genuinely alive with a fresh liveness beacon, reconciles native-Windows Git Bash wrapper pids to the watcher's recorded lock pid, and on native Windows treats drive-letter and MSYS path forms of the recorded home and watcher path as the same lock identity while POSIX keeps literal path equality.
It prints exactly one honest status line (`started` / `healthy` / `FAILED`, the last exiting non-zero) - never a false `already running` off a dying process.
The arm wrapper never drains `state/.wake-queue`; queue draining stays owned by `bin/fm-wake-drain.sh` and session start.
Its `--restart` mode signals only the watcher recorded in the current home's `state/.watch.lock`, so restarting one home cannot kill sibling secondmate watchers.
On native Windows with herdr-backed tasks, `fm-watch.sh` refreshes the same beacon before and after each slow herdr pane capture inside the stale-pane scan, not only at the poll-cycle boundary.
...
On native Windows with herdr, target discovery can fall back to the live Herdr owner recorded in the per-home session lock, and a stale explicit supervisor target can fall through to a live auto-detected target.
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while herdr uses native agent-state submit confirmation with an ANSI-aware composer classifier for guards and fallbacks.
...
New spawns select a backend from `--backend`, then `FM_BACKEND`, then local `config/backend`, then runtime auto-detection from `$TMUX`, `HERDR_ENV=1`, or cmux runtime signals, then the platform default: `tmux` on POSIX, `herdr` on Windows (tmux is POSIX-only).
...
Herdr is experimental and can be selected explicitly or by runtime auto-detection: treehouse remains the worktree provider for it exactly as it is for tmux (herdr is a session provider only), and its full verification - the container shape decision, created-vs-adopted default-tab prune safety, transactional spawn-abort cleanup, restored-layout husk respawn idempotency, stale-server compatibility refusal after herdr updates, verified CLI facts, a verified small-`--lines` capture bug and its workaround, and known gaps - is recorded in `docs/herdr-backend.md`.
...
For ship and scout work, `fm-spawn.sh` refuses to launch unless the resolved task path is a real git worktree root that is distinct from the project primary checkout and backed by the same git common directory as that project.
That same backing-store check catches stale or cross-wired treehouse pool entries that happen to point at another repository's valid worktree.
...
Task-selector `fm-send.sh fm-<id>` or `fm-send.sh <id>` requests to a live `kind=secondmate` are prefixed with the from-firstmate marker from `bin/fm-marker-lib.sh`, so the secondmate returns terse answers through status lines and detailed answers through docs plus status pointers instead of replying only in its own chat.
...
That proof uses `lsof` on POSIX and Restart Manager on native Windows, and every unavailable or error path fails closed.
...
The current watcher reliability work combines always-on bash triage with a durable queue for actionable wakes, a race-proof singleton lock, duplicate self-eviction, drain-time liveness assertion, native-Windows Git Bash pid reconciliation, and a self-verifying tracked-child arm wrapper.
