...
On native Windows under Git Bash the herdr adapter additionally converts paths between POSIX and Windows form (`cygpath` on the `--cwd` and the seeded task-pane `SHELL` env value it passes to herdr, and on the `foreground_cwd` it reads back) and launches the headless server detached; these conversions are gated on the Windows platform and leave POSIX behavior byte-identical (see [`docs/herdr-backend.md`](herdr-backend.md) "Native Windows path handling").
