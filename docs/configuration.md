...
For `fm-peek.sh`, an unmatched three-field explicit target such as `<herdr-session>:<workspace>:<pane>` routes through herdr only when herdr is active through `FM_BACKEND=herdr`, `config/backend`, or runtime auto-detection.
Plain POSIX unmatched explicit targets retain the historical tmux compatibility path.
...
On native Windows with the herdr supervisor backend, a stale `FM_SUPERVISOR_TARGET` is ignored when a live auto-detected fallback target resolves.
POSIX and tmux keep the explicit-target refusal path unchanged.
...
This fork previously carried a batch/sh polyglot `commands.test` here so a `cmd`-invoked gate would pin Git Bash off `git.exe`'s own install directory; it is dropped in favor of the intent-targeted contract above, and no gate command is platform-dispatched any more.
The native-Windows behavior run is instead owned by the Git Bash job in [`.github/workflows/ci.yml`](../.github/workflows/ci.yml), which runs `bin/fm-test-runner-windows.sh`: it runs most `tests/*.test.sh` in parallel (`FM_TEST_JOBS`, default `nproc`) with a fixed serial bucket, buffers and replays each test's output in glob order, and preserves any failing exit code.
