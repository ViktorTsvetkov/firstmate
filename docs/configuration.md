...
New spawns choose the backend in this order: an explicit `--backend` flag firstmate passes when it spawns a task, then `FM_BACKEND`, then the first non-empty line of local gitignored `config/backend`, then runtime auto-detection from `$TMUX`, `HERDR_ENV=1`, or cmux runtime signals, then the platform default: `tmux` on POSIX, `herdr` on Windows (tmux is POSIX-only, so an unconfigured Windows host falls back to herdr instead).
The same active-herdr checks, excluding the final platform default, decide whether an unrecorded three-field explicit target such as `<herdr-session>:<workspace>:<pane>` is routed through herdr by `fm-peek.sh`; on native Windows the existing herdr default still applies, while plain POSIX keeps the historical tmux path unless herdr is selected or auto-detected.
...
A herdr spawn additionally version-gates against the installed `herdr` binary's protocol, requires `jq`, and refuses to drive a running server that reports an incompatible protocol or restart-required state after a herdr update.
A stale running herdr server is not auto-restarted; restart it deliberately or switch to a fresh session before spawning again.
...
A backend spawn refusal from a missing dependency, version gate, stale running server, or unauthenticated socket is terminal for that selected backend; firstmate surfaces it as a blocker instead of silently retrying another backend.
...
Without overrides, backend detection uses `$TMUX_PANE` first, then `HERDR_ENV=1` with `HERDR_PANE_ID`, then a live native-Windows Herdr owner in `state/.lock`, then falls back to `tmux`.
That keeps a tmux pane nested inside herdr on the tmux transport, matching the runtime backend's innermost-first rule.
Target detection uses `FM_SUPERVISOR_TARGET`, then `$TMUX_PANE`, then `"${HERDR_SESSION:-default}:${HERDR_PANE_ID}"` under herdr, then a live native-Windows Herdr session-lock pane, then the legacy `firstmate:0` tmux fallback with a warning.
On native Windows with the herdr supervisor backend, a stale `FM_SUPERVISOR_TARGET` is ignored when a live auto-detected fallback target resolves; POSIX and tmux keep the explicit-target refusal path unchanged.
...
In that shape, when `HERDR_ENV=1`, `HERDR_PANE_ID` is present, `herdr pane get` returns a `terminal_id`, and `herdr agent get` confirms the pane's detected agent type is a verified harness, `fm-lock.sh` records the Herdr session, `pane_id`, and `terminal_id` instead.
...
On native Windows, the propagation helper normalizes drive-letter path forms before deriving the destination's repo-relative path for the gitignore guard.
...
FM_BACKEND_HERDR_COMPOSER_LINES=20  # herdr-only: tail lines scanned to locate the composer row for guards and submit fallback
FM_BACKEND_HERDR_IDLE_RE='^Type a message\.\.\.$'  # herdr-only: empty-composer placeholder regex after border/prompt stripping
FM_BACKEND_HERDR_CODEX_IDLE_RE='^(Run /review on my current changes|Improve documentation in @[^[:space:]]+|Find and fix a bug in @[^[:space:]]+|Write tests for @[^[:space:]]+|Summarize recent commits|Implement \{feature\})$'  # herdr-only: Windows Codex idle suggestions treated as empty after the bare prompt is stripped
FM_BACKEND_HERDR_BARE_PROMPT_RE='^[❯›]'  # herdr-only: bare composer prompt glyphs recognized outside a bordered composer row
FM_BACKEND_HERDR_SUBMIT_POLLS=6  # herdr-only: agent-status polls after Enter when an idle baseline should start a turn
FM_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0.6  # herdr-only: minimum agent-status confirmation budget after Enter
...
FM_BOOTSTRAP_DETECT_ONLY=0   # internal/read-only session-start mode: skip Windows CLAUDE.md materialization and bootstrap's mutating sweeps, and print advisory TANGLE wording
...
FM_GUARD_GRACE=300      # seconds before guard warnings, arm health checks, and the Claude turn-end guard treat a watcher/AFK daemon beacon as stale
FM_ARM_CONFIRM_TIMEOUT=10   # seconds fm-watch-arm waits to confirm a fresh watcher before reporting FAILED; native Windows defaults to 30
FM_WATCH_ARM_CHILD_PID_FILE=   # internal native-Windows arm/watch handshake path; set only by fm-watch-arm.sh so it can reconcile Git Bash wrapper pids to the watcher BASHPID
...
FM_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS=2 # seconds fm-teardown.sh waits before retrying a lock-blocked safety check or worktree return
...
FM_SUPERVISOR_BACKEND=             # optional supervisor pane backend override; tmux/herdr only, otherwise detects $TMUX_PANE then HERDR_ENV/HERDR_PANE_ID then native-Windows Herdr session lock before tmux fallback
FM_SUPERVISOR_TARGET=              # optional supervisor pane target override; tmux target or herdr <session>:<pane-id>, otherwise auto-detected; stale native-Windows Herdr overrides may fall through to a live auto-detected target
