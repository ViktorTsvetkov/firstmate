...
test_windows_last_status_line_strips_leading_utf8_bom() {
  ...
  [ "$line" = "done: something" ] || fail "forced-Windows leading UTF-8 BOM was not stripped from the last status line"
  (FM_PLATFORM_IS_WINDOWS=yes; status_is_captain_relevant "$line") || fail "forced-Windows BOM-prefixed done: did not classify as captain-relevant after normalization"
...
test_windows_last_status_line_ignores_bom_on_non_last_line() {
  ...
  line=$(FM_PLATFORM_IS_WINDOWS=yes; last_status_line "$d/task.status")
  [ "$line" = "done: finished" ] || fail "forced-Windows last_status_line did not preserve the later terminal status"
  (FM_PLATFORM_IS_WINDOWS=yes; status_is_captain_relevant "$line") || fail "forced-Windows later done: line did not classify as captain-relevant"
...
test_windows_last_status_line_decodes_utf16le_done() {
  ...
  (FM_PLATFORM_IS_WINDOWS=yes; status_is_captain_relevant "$line") || fail "forced-Windows UTF-16LE done: did not classify as captain-relevant"
...
test_windows_last_status_line_decodes_utf16be_done() {
  ...
  (FM_PLATFORM_IS_WINDOWS=yes; status_is_captain_relevant "$line") || fail "forced-Windows UTF-16BE done: did not classify as captain-relevant"
...
test_forced_windows_utf8_status_is_unchanged() {
  ...
  (FM_PLATFORM_IS_WINDOWS=yes; status_is_captain_relevant "$line") || fail "forced-Windows UTF-8 done: did not classify as captain-relevant"
  pass "forced-Windows UTF-8 status logs classify unchanged"
}

test_windows_bom_prefixed_direct_status_line_classifies_with_anchor() {
  local line
  line=$(printf '\357\273\277done: direct')
  (FM_PLATFORM_IS_WINDOWS=yes FM_CAPTAIN_RE='^done:' status_is_captain_relevant "$line") \
    || fail "forced-Windows direct BOM-prefixed done: did not classify with anchored captain regex"
  pass "forced-Windows direct status-line classification strips a leading UTF-8 BOM"
}
