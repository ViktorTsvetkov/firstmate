...
#   (x) Windows Restart Manager no-holder proof               -> lock removed, ALLOW
#   (y) Windows Restart Manager holder/error/fresh lock        -> lock kept, REFUSE
set -u
...
case "$mode" in
  none) exit 1 ;;
  holder) exit 0 ;;
  error) echo "Restart Manager simulated failure" >&2; exit 2 ;;
esac
echo "unknown Restart Manager probe mode: $mode" >&2
exit 2
SH
  chmod +x "$case_dir/fakebin/powershell.exe"
}
...
test_windows_restart_manager_stale_index_lock_cleared() {
  local case_dir rc lock
  case_dir=$(make_case windows-rm-stale-index-lock)
  write_meta "$case_dir" no-mistakes ship
  wt_commit "$case_dir" "shippable work"
  git -C "$case_dir/wt" push -q origin fm/task-x1
  git -C "$case_dir/project" fetch -q origin
  ...
  add_lock_aware_treehouse "$case_dir"
  add_restart_manager_probe "$case_dir" none
  ...
  set +e
  FM_PLATFORM_IS_WINDOWS=yes FM_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS=0 FM_STALE_WORKTREE_LOCK_AGE_SECS=1 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e

  expect_code 0 "$rc" "windows-rm-stale-index-lock: teardown should clear stale lock when Restart Manager reports no holders"
  assert_grep "removed provably-stale git lock" "$case_dir/stderr" \
    "windows-rm-stale-index-lock: teardown did not report clearing the stale lock"
  assert_absent "$lock" "windows-rm-stale-index-lock: stale lock file should have been removed"
  pass "Windows Restart Manager no-holder proof clears an old index.lock"
}

test_windows_restart_manager_live_holder_refuses() {
  local case_dir rc lock
  case_dir=$(make_case windows-rm-live-index-lock)
  write_meta "$case_dir" no-mistakes ship
  wt_commit "$case_dir" "shippable work"
  git -C "$case_dir/wt" push -q origin fm/task-x1
  git -C "$case_dir/project" fetch -q origin
  ...
  add_lock_aware_treehouse "$case_dir"
  add_restart_manager_probe "$case_dir" holder
  ...
  expect_code 1 "$rc" "windows-rm-live-index-lock: teardown should refuse when Restart Manager reports a holder"
  assert_grep "not provably stale" "$case_dir/stderr" \
    "windows-rm-live-index-lock: teardown did not explain the refusal"
  [ -e "$lock" ] || fail "windows-rm-live-index-lock: live-held lock file was removed"
  pass "Windows Restart Manager holder result leaves index.lock in place"
}

test_windows_restart_manager_fresh_lock_refuses() {
  local case_dir rc lock
  case_dir=$(make_case windows-rm-fresh-index-lock)
  write_meta "$case_dir" no-mistakes ship
  wt_commit "$case_dir" "shippable work"
  git -C "$case_dir/wt" push -q origin fm/task-x1
  git -C "$case_dir/project" fetch -q origin
  ...
  set +e
  FM_PLATFORM_IS_WINDOWS=yes FM_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS=0 FM_STALE_WORKTREE_LOCK_AGE_SECS=999999 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e

  expect_code 1 "$rc" "windows-rm-fresh-index-lock: teardown should refuse a too-fresh lock"
  assert_grep "not provably stale" "$case_dir/stderr" \
    "windows-rm-fresh-index-lock: teardown did not explain the refusal"
  [ -e "$lock" ] || fail "windows-rm-fresh-index-lock: fresh lock file was removed"
  pass "Windows Restart Manager no-holder result does not bypass the lock age gate"
}

test_windows_restart_manager_error_refuses() {
  local case_dir rc lock
  case_dir=$(make_case windows-rm-error-index-lock)
  write_meta "$case_dir" no-mistakes ship
  wt_commit "$case_dir" "shippable work"
  git -C "$case_dir/wt" push -q origin fm/task-x1
  git -C "$case_dir/project" fetch -q origin
  ...
  add_lock_aware_treehouse "$case_dir"
  add_restart_manager_probe "$case_dir" error
  ...
  expect_code 1 "$rc" "windows-rm-error-index-lock: teardown should refuse when Restart Manager errors"
  assert_grep "Restart Manager check failed" "$case_dir/stderr" \
    "windows-rm-error-index-lock: teardown did not report the Restart Manager failure"
  [ -e "$lock" ] || fail "windows-rm-error-index-lock: lock file was removed after Restart Manager failed"
  pass "Windows Restart Manager errors leave index.lock in place"
}
