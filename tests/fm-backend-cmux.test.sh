...
test_capture_forced_posix_preserves_jq_crlf() {
  case "$(uname -s)" in
    MINGW*|MSYS*|CYGWIN*)
      echo "skip: WINDOWS-DEFER fm_backend_cmux_capture forced-POSIX exact-jq-CRLF - asserts POSIX-jq byte output, but native Windows jq does text-mode CRLF translation"
      return 0 ;;
  esac
  local dir fb out
