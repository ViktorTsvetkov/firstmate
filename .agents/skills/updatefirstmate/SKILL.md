...
1. **Run the updater:**
   ...
   bin/fm-update.sh
   ```
   It fast-forwards this firstmate repo's default branch from origin, then fast-forwards every registered secondmate home (leased treehouse worktree, plain worktree, or standalone clone) the same way.
...
2. **Re-read AGENTS.md if your own instructions changed.**
   ...
   **Read `AGENTS.md` now** (`CLAUDE.md` is a symlink to it on POSIX and a native-Windows mirror when bootstrap materialized it) to refresh your operating instructions before doing anything else, so you are acting on the new instructions rather than the stale ones you were started with.
