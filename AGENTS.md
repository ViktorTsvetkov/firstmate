...
1. **Never write to a project.**
   ...
   Seven sanctioned write exceptions are indexed here; their procedures live where they are used: tool-driven project initialization (section 6), native-Windows `CLAUDE.md` materialization via `bin/fm-bootstrap.sh` (section 3), fleet sync via `bin/fm-fleet-sync.sh` (sections 3 and 7), local-HEAD secondmate sync via `bin/fm-bootstrap.sh` and `bin/fm-spawn.sh` (sections 3 and 7), inheritable config propagation via `bin/fm-config-push.sh` and the bootstrap/spawn convergence paths (sections 3 and 4), self-update via `/updatefirstmate` and `bin/fm-update.sh` (section 12), and approved `local-only` merge via `bin/fm-merge-local.sh` (section 7).
   All are fast-forward operations, guarded local file materialization, guarded gitignored-config propagation, or guarded local merges that never force, stash, or discard unlanded work.
...
```
AGENTS.md            this file (CLAUDE.md is a symlink to it on POSIX; native-Windows bootstrap may materialize a regular mirror)
...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then the platform default (tmux on POSIX, herdr on Windows since tmux is POSIX-only); tmux is the verified reference backend (docs/tmux-backend.md), while herdr, zellij, orca, and cmux are experimental spawn backends (docs/herdr-backend.md, docs/zellij-backend.md, docs/orca-backend.md, docs/cmux-backend.md) - herdr and cmux can also be selected by runtime auto-detection, zellij and orca never are (always explicit); not inherited into secondmate homes
...
state/               volatile runtime signals; gitignored
  .lock  per-home session lock (docs/configuration.md "Session lock state")
...
2. **Bootstrap** - detect-only diagnostics (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run and always print.
   ...
   Native-Windows `CLAUDE.md` materialization and the three MUTATING sweeps - fleet sync, the local secondmate fast-forward sweep, and X-mode artifact writes - run only when this session actually holds the lock from step 1.
...
When locked, bootstrap first materializes `CLAUDE.md` as a real mirror of `AGENTS.md` on native Windows if Git checked out the tracked symlink as a placeholder text file; it marks the path skip-worktree so the intentional Windows divergence does not dirty porcelain.
Then it runs the mutating sweeps: fleet sync via `bin/fm-fleet-sync.sh`, best-effort and non-fatal, under the hard-rule exception in section 1 (set `FM_FLEET_PRUNE=0` to temporarily disable that branch pruning); and a sweep of every live secondmate home, fast-forwarding each one's worktree to firstmate's own current default-branch commit so the fleet stays converged on whatever version firstmate is on.
...
- `CLAUDE_MD: skipped|failed: <reason>` - bootstrap could not safely materialize the native-Windows `CLAUDE.md` mirror or could not hide the intentional mirror from porcelain.
  Treat failures as blocking for native-Windows Claude sessions; skipped divergent files need human inspection before firstmate changes them.
...
The task selector (`fm-<id>` or exact `<id>`) resolves through this home's `state/<id>.meta`; pass an explicit backend target only when intentionally targeting an endpoint outside this firstmate home.
...
bin/fm-spawn.sh <id> projects/<repo> --backend herdr  # experimental herdr backend (docs/herdr-backend.md); version/stale-server gates at spawn
...
In all cases, the script asserts the resolved worktree is a genuine isolated worktree distinct from the primary checkout and backed by the same git common directory as the requested repo (aborting the spawn otherwise, to prevent the worktree tangle of section 8 or a stale treehouse pool cross-wire), installs the turn-end hook, records `state/<id>.meta`, and launches the agent with the brief.
...
Herdr teardown on native Windows must release an empty herdr session through the adapter's guarded session-release path; the process-leak evidence and exact cleanup semantics live in `docs/herdr-backend.md`.
...
On native Windows, that live-lock check treats drive-letter and MSYS path forms of the same home and watcher path as the same identity; POSIX path comparison stays literal.
...
On native Windows with herdr-backed tasks, it also refreshes that beacon around slow pane reads during the stale-pane scan, so a cycle with multiple slow `herdr pane read` calls does not look dead to the guard before the next poll boundary.
...
Two further guards prevent the tangle upstream: `fm-spawn` refuses to launch unless treehouse or Orca yields a genuine isolated worktree distinct from the primary checkout and backed by the requested repo's git common directory, and every ship brief's first instruction has the crewmate verify it is in its own worktree before branching (section 11).

On the `claude` harness, "no turn ends blind" has a structural backstop beyond the pull-based `fm-guard.sh` banner: `bin/fm-turnend-guard.sh`, a Claude Code Stop hook registered in the tracked `.claude/settings.json`, fires on every primary turn end and, when tasks are in flight without either a live identity-matched watcher lock with a fresh beacon or healthy AFK daemon-owned supervision, blocks the stop (exit 2 with a reason) so the turn cannot end without acting on it first.
It shares status fields with `fm-guard.sh` via `bin/fm-supervision-lib.sh`, uses `bin/fm-wake-lib.sh` for live watcher lock health and AFK daemon-owned supervision health, and never blocks more than once per turn (Claude Code's own `stop_hook_active` loop-guard field lets it always allow the retry).
